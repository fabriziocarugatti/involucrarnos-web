      ...
      >
        ...
        <div className="p-6 md:p-7 flex-1 flex flex-col">
          <div className="flex items-center gap-2 mb-3 flex-wrap">
            <span className="text-[0.62rem] font-bold tracking-[0.18em] uppercase text-dorado-deep">
              {study.category}
            </span>
          ...
          <p className="text-texto/65 text-sm leading-relaxed line-clamp-3 mb-5 flex-1">
            {study.bajada}
          </p>

          {/* Supporting stats mini-row */}
          {supportingStats.length > 0 && (
            <div className="grid grid-cols-2 gap-2.5 pt-4 border-t border-black/8 mb-5">
              {supportingStats.map((s, i) => (
                <div key={i}>
                  <p className="font-title font-800 text-azul-dark text-base leading-none tabular-nums">
                    {s.value}
                  </p>
                  <p className="text-[0.65rem] text-texto/50 mt-1 leading-tight line-clamp-2">
                    {s.label}
                  </p>
                </div>
              ))}
